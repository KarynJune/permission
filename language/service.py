...
def get_data(page, product_code):
    ...
    query = Q(source__istartswith='taptap', post_date__lt='2018-10-24') & ~Q(score=0)
    posts = Post.objects.using(product_code).extra(where=['LENGTH(subject)<=500']).filter(query).order_by("-post_date")[(page - 1) * 100:100 * page]
